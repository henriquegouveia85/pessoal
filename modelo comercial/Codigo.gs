...
// ============================================================
// Altere PASTA_ID para o ID da pasta do seu evento no Google Drive
// A senha do admin é definida no config.js (admin.senha)
const PASTA_ID = 'COLOQUE_AQUI_O_ID_DA_PASTA';
const ARQUIVO_PAUSA = 'galeria_pausada.txt';
const SENHA_ADMIN = 'cpw2026';
const NOME_EVENTO = 'Galeria Ao Vivo';
...
function doPost(e) {
  try {
    ...
    if (acao === 'verificar_senha') {
      ...
      return ContentService.createTextOutput(JSON.stringify({sucesso: senha === SENHA_ADMIN})).setMimeType(ContentService.MimeType.JSON);
    }

    if (acao === 'pausar' || acao === 'retomar') {
      const pausar = acao === 'pausar';
    ...
    if (acao === 'exportar') {
      try {
        ...
        let relatorio = 'RELATÓRIO DO EVENTO - ' + NOME_EVENTO + '\n';
        ...
        // CSV
        let csv = 'Nome do Convidado,Mensagem,Data,Link da Foto\n';
        fotos.forEach(f => {
          const nomeCSV = (f.nomeConvidado || '').replace(/"/g, '""');
          const msgCSV = (f.mensagem || '').replace(/"/g, '""');
          csv += '"' + nomeCSV + '","' + msgCSV + '","' + f.data + '","' + f.url + '"\n';
        });

        const blob = Utilities.newBlob(relatorio, 'text/plain', 'Relatorio_' + NOME_EVENTO.replace(/\s+/g, '_') + '_' + new Date().getTime() + '.txt');
        const arquivoRelatorio = pasta.createFile(blob);
        return ContentService.createTextOutput(JSON.stringify({
          sucesso: true, csv: csv,
